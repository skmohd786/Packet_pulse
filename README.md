...
- **Uptime Calculation**: Computes live percentage uptime per domain based on historical check metrics.
- **PostgreSQL Metric Storage**: Persists monitor configurations, real-time metrics, and incident records.
...
- **Live Response Time Chart**: Interactive latency stream graph built with Recharts.
...
- **Database**: PostgreSQL (`pg` module) with schema initialization (`schema.sql`)
...
- (Optional) PostgreSQL database instance running locally on port 5432. If PostgreSQL is not active, PacketPulse auto-engages its built-in in-memory SQL state manager.
...
- `GET /api/monitors/:id/metrics` — Fetch historical metric pings for charts
