...
import { AnalyticsPage, IncidentsPage, NodesPage, OverviewPage, SettingsPage, TrafficPage } from './components/PacketPulsePages';
...
export const App: React.FC = () => {
  ...
  return (
    <div className="pp-app min-h-screen bg-zinc-950 text-zinc-100 flex flex-col font-sans pb-10">
      <Header
        monitors={monitors}
        incidents={incidents}
        wsStatus={wsStatus}
      ...
      <main className="pp-main max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 flex-1 w-full space-y-6">
        {activePage === 'overview' ? (
          <OverviewPage monitors={monitors} incidents={incidents} metrics={currentMetrics} onSelectMonitor={(id) => setSelectedMonitorId(id)} onResolve={handleResolveIncident} onOpenAddModal={() => setIsAddModalOpen(true)} />
        ) : activePage === 'traffic' ? (
          <TrafficPage monitors={monitors} incidents={incidents} metrics={currentMetrics} onSelectMonitor={(id) => setSelectedMonitorId(id)} onResolve={handleResolveIncident} onOpenAddModal={() => setIsAddModalOpen(true)} />
        ) : activePage === 'analytics' ? (
          <AnalyticsPage monitors={monitors} incidents={incidents} metrics={currentMetrics} onSelectMonitor={(id) => setSelectedMonitorId(id)} onResolve={handleResolveIncident} onOpenAddModal={() => setIsAddModalOpen(true)} />
        ) : activePage === 'nodes' ? (
          <NodesPage monitors={monitors} incidents={incidents} metrics={currentMetrics} onSelectMonitor={(id) => setSelectedMonitorId(id)} onResolve={handleResolveIncident} onOpenAddModal={() => setIsAddModalOpen(true)} />
        ) : activePage === 'incidents' ? (
          <IncidentsPage monitors={monitors} incidents={incidents} metrics={currentMetrics} onSelectMonitor={(id) => setSelectedMonitorId(id)} onResolve={handleResolveIncident} onOpenAddModal={() => setIsAddModalOpen(true)} />
        ) : activePage === 'settings' ? (
          <ProfileSettings monitors={monitors} incidents={incidents} />
        ) : (
          <OverviewPage monitors={monitors} incidents={incidents} metrics={currentMetrics} onSelectMonitor={(id) => setSelectedMonitorId(id)} onResolve={handleResolveIncident} onOpenAddModal={() => setIsAddModalOpen(true)} />
        )}
      </main>
