...
import { AlertCircle, CheckCircle2, ShieldAlert, ChevronDown, ChevronUp } from 'lucide-react';
...
export const IncidentDetails: React.FC<IncidentDetailsProps> = ({ incidents, onResolve }) => {
  const [expandedIncidentId, setExpandedIncidentId] = useState<string | number | null>(null);
    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-5">
      <div className="flex items-center justify-between mb-3.5">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-mono font-bold text-white">Incidents & AI Observability Intelligence</h3>
        </div>
      ...
      {incidents.length === 0 ? (
        ...
        <div className="space-y-2.5 max-h-96 overflow-y-auto pr-1">
          {incidents.map((incident) => {
            ...
            const titleText = incident.title || incident.message || `Incident on ${incident.domain || 'Target'}`;
            const isExpanded = expandedIncidentId === incId;

            return (
              <div
                key={String(incId)}
                className={`p-3.5 rounded border text-xs font-mono transition-all ${
                  isOpen
                    ? 'bg-rose-950/20 border-rose-800/60'
                    : 'bg-zinc-950 border-zinc-800'
                }`}
              >
                <div className="flex items-start justify-between gap-3 cursor-pointer" onClick={() => toggleExpand(incId)}>
                ...
                {(isExpanded || isOpen) && <AIInsight incident={incident} />}
              </div>
            );
          })}
        </div>
